...
from database import fetch_data_from
...
def obter_infos_tabelas(tabela):
    return fetch_data_from(tabela)

def seleciona_tabela(tabela):
    df = obter_infos_tabelas(tabela).reset_index()
    return df[df['Selecionar'] == 1].reset_index(drop=True)
...
def escrever_pagina_documentacao(tabela, sistema_origem, nome_descritivo, desc_tabela):
    markdown_template = '''\
## Nome tabela
`{nome_descritivo}`
...
{{{{ example("bases/examples/{tabela}.csv") }}}}
'''
...
def extrair_colunas_aliased_sql(arquivo_sql):
    with open(f'querys/{arquivo_sql}.sql', 'r') as arquivo:
        conteudo = arquivo.read()

    padrao = re.compile(r'(\w+)\s+AS\s+(\w+)', re.IGNORECASE)
    colunas_aliased = dict(padrao.findall(conteudo))
    return colunas_aliased

def update_tabela_pela_query(nome_descritivo, tabela):
        columns: dict = extrair_colunas_aliased_sql(nome_descritivo)
        df_tabela_renomeada = fetch_data_from(tabela)
        df_tabela_renomeada.loc[~df_tabela_renomeada.index.isin(list(columns.keys())), 'Selecionar'] = False
        for k, v in columns.items():
            df_tabela_renomeada.loc[k, 'SCRTEXT_M'] = v

        update_table(tabela, df_tabela_renomeada)
        logger.info(f"Tabela {tabela} atualizada pela query {nome_descritivo}!")

def gerar_documentacao_tabela(tabela, nome_descritivo, sistema_origem, desc_tabela, query=False):
    
    if query:
        update_tabela_pela_query(nome_descritivo, tabela)
        df_tabela = seleciona_tabela(tabela)
        df_tabela_renomeada = df_tabela.reset_index()
    else:
        df_tabela = seleciona_tabela(tabela)
    ...
    escrever_pagina_documentacao(tabela, sistema_origem, nome_descritivo, desc_tabela)
